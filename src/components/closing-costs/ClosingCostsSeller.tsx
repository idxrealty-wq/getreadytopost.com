...
import { calculateSellerCosts, calculateTotalSellerCosts } from '@/lib/closing-costs/calc';
...
export default function ClosingCostsSeller({ state }: Props) {
  const costs = calculateSellerCosts(state);
  const total = calculateTotalSellerCosts(state);

  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h2 className="text-lg font-bold mb-3">Seller Closing Disclosure</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="border-t-2 border-gray-300 font-bold">
              <td className="py-2 px-2">Total Seller Costs</td>
