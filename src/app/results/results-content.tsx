...
const gradeColors: Record<string, string> = { 'A+': 'from-green-400 to-emerald-600', A: 'from-green-400 to-emerald-600', B: 'from-blue-400 to-blue-600', C: 'from-yellow-400 to-orange-500', D: 'from-red-400 to-red-600' };
const gradeBarWidth: Record<string, string> = { 'A+': 'w-[100%]', A: 'w-[95%]', B: 'w-[75%]', C: 'w-[50%]', D: 'w-[25%]' };
const gradeBg: Record<string, string> = { 'A+': 'bg-green-500', A: 'bg-green-500', B: 'bg-blue-500', C: 'bg-yellow-500', D: 'bg-red-500' };
const categoryLabels: Record<string, string> = { headline: 'Headline Quality', length: 'Description Length', emotion: 'Emotional Appeal', keywords: 'Keywords and SEO', cta: 'Call to Action', professionalism: 'Professionalism' };
...
export default function ResultsContent() {
  ...
  const saveToVault = async (currentUser: User) => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, 'submissions', submissionId), { userId: currentUser.uid, savedToVault: true, savedAt: new Date().toISOString() });
  ...
  return (
    <>
      <main className="pt-20 min-h-screen relative" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2000)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
          ...
          {loading ? (
            <div className="text-center py-20">
              ...
              <div className="bg-white/10 rounded-2xl p-6 border border-white/20 max-w-md mx-auto">
                ...
                <p className="text-gray-400 text-sm">This page will update automatically once payment is confirmed.</p>
              </div>
            </div>
          ) : submission?.analysis ? (
            <div className={'transition-all duration-1000 ' + (revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10')}>
                  ...
                  <div className="max-w-2xl mx-auto bg-gradient-to-r from-[#c9a227] to-[#b8911f] rounded-2xl p-5 shadow-2xl border-2 border-white/20">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        <button onClick={() => setShowAuthModal(true)} className="bg-white text-[#c9a227] px-5 py-2 rounded-xl font-bold text-sm hover:bg-gray-100 transition whitespace-nowrap">Save to Vault</button>
              ...
              {!submission.savedToVault ? (
                <div className="bg-gradient-to-r from-[#1a2b4a] to-[#2d4a7c] border-2 border-[#c9a227]/60 rounded-2xl p-6 mb-8 shadow-2xl">
                  <div className="flex flex-col md:flex-row items-center gap-4">
                    ...
                    <button onClick={handleSaveToVault} disabled={saving} className="bg-[#c9a227] hover:bg-[#b8911f] text-white px-8 py-3 rounded-xl font-bold text-lg transition disabled:opacity-50 whitespace-nowrap shadow-xl">
                      {saving ? 'Saving...' : user ? 'Save to My Vault' : 'Create Free Account and Save'}
                    </button>
                  </div>
                  {!user && (
                    <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                      <div className="bg-white/5 rounded-lg p-2 text-xs text-gray-300">Free account</div>
                      <div className="bg-white/5 rounded-lg p-2 text-xs text-gray-300">Agent Vault access</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
                {Object.entries(submission.analysis.categories).map(([key, val], i) => (
                  <div key={key} className="bg-white/10 backdrop-blur-md rounded-xl p-5 border border-white/20 shadow-xl" style={{ transitionDelay: i * 150 + 'ms', opacity: revealed ? 1 : 0 }}>
              ...
              <div className="bg-gradient-to-br from-emerald-900/60 to-green-900/40 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-500/40 shadow-2xl mb-10 relative">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={copyRewrite} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition shadow-lg">
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
