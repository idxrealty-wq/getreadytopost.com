          ...
          <div className={`transition-all duration-1000 \${revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
              {Object.entries(submission.analysis.categories).map(([key, val], i) => (
                <div 
                  key={key} 
                  className="bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/10 transition-all duration-500"
            ...
            <div className="bg-gradient-to-br from-emerald-900/50 to-green-900/30 rounded-2xl p-8 border-2 border-emerald-500/30 mb-10 relative">
              ...
              <div className="mt-4 flex items-center gap-2">
                <span className="bg-emerald-600/30 text-emerald-300 text-xs font-semibold px-3 py-1 rounded-full">MLS-Ready</span>
                <span className="bg-emerald-600/30 text-emerald-300 text-xs font-semibold px-3 py-1 rounded-full">Fair Housing Compliant</span>
                <span className="bg-emerald-600/30 text-emerald-300 text-xs font-semibold px-3 py-1 rounded-full">SEO Optimized</span>
              </div>
            </div>

            {/* Recommendations */}
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/10 mb-10">
              ...
              <div className="space-y-4">
                {submission.analysis.recommendations.map((rec, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-[#c9a227] flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <div className="text-center">
              <Link 
                ...
                className="inline-block bg-red-500 hover:bg-red-600 text-white px-8 py-4 rounded-xl font-bold text-lg transition"
              >
