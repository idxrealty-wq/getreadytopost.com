...
const categoryLabels: Record<string, string> = {
  ...
  keywords: "Keywords and SEO",
  cta: "Call to Action",
  professionalism: "Professionalism",
};

export default function ResultsPaginated() {
  ...
  const saveToVault = async (currentUser: User) => {
    ...
    try {
      ...
      setSubmission((prev) =>
        prev ? { ...prev, userId: currentUser.uid, savedToVault: true } : null
      );
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-10">
          {loading && !isCredit && !submission?.analysis ? (
            ...
            <div className="text-center py-20">
              <div className="w-20 h-20 border-4 border-[#c9a227] border-t-transparent rounded-full animate-spin mx-auto mb-8"></div>
              <h2 className="text-3xl font-bold text-white mb-4">Processing Your Listing...</h2>
              <p className="text-gray-300 text-lg mb-2">Our AI is grading your listing across 6 categories</p>
              <p className="text-gray-400 mb-8">This usually takes 30-60 seconds</p>
              <div className="bg-white/10 rounded-2xl p-6 border border-white/20 max-w-md mx-auto">
                <p className="text-white font-bold mb-2">Credit Applied</p>
                <p className="text-gray-400 text-sm">Generating your report now...</p>
              </div>
            </div>
          ) : submission?.analysis ? (
            <div className={`transition-all duration-1000 ${revealed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
              ...
              {!submission.savedToVault ? (
                <div className="bg-gradient-to-r from-[#1a2b4a] to-[#2d4a7c] border-2 border-[#c9a227]/60 rounded-2xl p-6 mb-8 shadow-2xl">
                  <div className="flex flex-col md:flex-row items-center gap-4">
                    ...
                    <button className="bg-[#c9a227] hover:bg-[#b8911f] text-white px-8 py-3 rounded-xl font-bold text-lg transition disabled:opacity-50 whitespace-nowrap shadow-xl" disabled={saving} onClick={handleSaveToVault}>
              ...
              <div className="flex items-center justify-between gap-3 mb-8">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white text-sm font-semibold border border-white/15 disabled:opacity-40 disabled:cursor-not-allowed"
                  ...
                  <button
                    className="px-4 py-2 rounded-lg bg-[#c9a227] hover:bg-[#b8911f] text-white text-sm font-bold shadow-lg disabled:opacity-40 disabled:cursor-not-allowed"
                ...
                <div>
                  <div className="bg-gradient-to-br from-emerald-900/60 to-green-900/40 backdrop-blur-md rounded-2xl p-8 border-2 border-emerald-500/40 shadow-2xl mb-10 relative">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition shadow-lg" onClick={copyRewrite}>
                        {copied ? "Copied!" : "Copy"}
                      </button>
                    </div>
              ...
              {currentPage === 3 && (
                <div className="text-center mb-20">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 justify-center">
                    ...
                    {user && (
                      <Link className="inline-block bg-[#c9a227] hover:bg-[#b8911f] text-white px-8 py-4 rounded-xl font-bold text-lg transition shadow-xl" href="/agent-vault">
                        My Agent Vault
                      </Link>
                    )}
                  </div>
