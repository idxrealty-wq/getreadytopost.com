...
const gradingCategories = [
  ...
  { icon: "🏠", title: "Features", description: "Are your key property details highlighted in a way that excites?", color: "bg-green-500/20 border-green-400/40" },
  { icon: "🌅", title: "Lifestyle", description: "Does your description paint a picture of what it feels like to live there?", color: "bg-purple-500/20 border-purple-400/40" },
  { icon: "⚖️", title: "Compliance", description: "Is your language Fair Housing compliant and MLS-safe?", color: "bg-red-500/20 border-red-400/40" },
  ...
  { icon: "📣", title: "Call to Action", description: "Does it drive the buyer to schedule a showing?", color: "bg-pink-500/20 border-pink-400/40" },
];
...
export default function RateMyListingPage() {
  ...
  const handleSubmit = async () => {
    if (!user?.uid) { setError("Please sign in to submit a listing."); return; }
    if (!email || !listing) { setError("Please fill in email and listing description."); return; }
    ...
    try {
      ...
      if ((creditBalance ?? 0) > 0) {
        await fetch("/api/credits/deduct", {
          ...
          body: JSON.stringify({ userId: user.uid, listingId: docRef.id }),
        });

        fetch("/api/submissions/run-analysis", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 shadow-2xl mb-8">
          <div className="mb-8">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:border-[#c9a227]" />
          </div>
          <div className="mb-8">
            <label className="block text-white font-bold mb-3">Your Listing Description</label>
            <textarea value={listing} onChange={(e) => setListing(e.target.value)} placeholder="Paste your MLS listing description here..." rows={8} className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:border-[#c9a227] resize-none" />
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            <button onClick={handleSubmit} disabled={loading || !user?.uid} className="flex-1 bg-[#c9a227] hover:bg-[#b8911f] disabled:opacity-50 text-white px-8 py-4 rounded-xl font-bold text-lg transition shadow-xl">
              {loading ? "Submitting..." : creditBalance && creditBalance > 0 ? `Use 1 Credit (${creditBalance} left)` : "Buy Credits & Analyze"}
            </button>
            <Link href="/" className="flex-1 bg-white/10 hover:bg-white/20 text-white px-8 py-4 rounded-xl font-bold text-lg transition text-center">Back Home</Link>
          </div>
