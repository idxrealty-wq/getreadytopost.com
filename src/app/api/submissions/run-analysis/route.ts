    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an elite MLS listing copywriter. Rewrite to exactly 155 words (145-165 allowed). Return ONLY the rewritten text.',
          },
          {
            role: 'user',
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Grade original
    const originalSystem = `You are a strict MLS listing grader. Grade ONLY the ORIGINAL listing across 6 categories: headline, length, emotion, keywords, cta, compliance.
...
Return ONLY valid JSON with this shape:
{
  "categories": {
    ...
    console.error('RUN_ANALYSIS_ERROR:', e?.message, e?.stack);
