  ...
  try {
    ...
    if (email && process.env.RESEND_API_KEY) {
      try {
        ...
        await resend.emails.send({ from: "onboarding@resend.dev", to: email, subject: `Your GetReadyToPost Report - Grade ${overall}`, html });
        await submissionRef.update({ emailSendStatus: "sent", emailSentAt: new Date().toISOString() });
      } catch (e: any) {
        console.error("Email send error:", e);
        await submissionRef.update({ emailSendStatus: "error", emailSendError: String(e?.message || e) });
      }
    } else {
      await submissionRef.update({ emailSendStatus: email ? "missing_resend_key" : "missing_email" });
    }
