    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an elite MLS listing copywriter. Use ONLY the provided facts. Rewrite to exactly 155 words (145-165 allowed). Return ONLY the rewritten text.',
          },
          {
            role: 'user',
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = `You are an elite MLS listing rewriter and strict grader. Grade the original across 6 categories. Produce a rewrite that earns an A. Use ONLY facts from the FACTS block. Do NOT invent beds, baths, sqft, lot size, year built, HOA, views, waterfront, renovations, appliances, school zones, or distances. WORD COUNT RULE: Rewrite must be 145-165 words. CONTENT WEIGHT: 50% HOME INTERIOR, 20% HOME EXTERIOR, 20% LIFESTYLE AND FEATURES, 10% LOCATION. COMPLIANCE RULES: Use "primary bedroom" not "master". MLS-safe and Fair Housing compliant. No invented facts. GRADING SCALE: A=all criteria met, B=mostly good with gaps, C=needs significant work, D=poor, F=unacceptable. Return ONLY valid JSON. OUTPUT JSON SHAPE: {"overall":"A|B|C|D|F","originalGrade":"A|B|C|D|F","rewrite":"145-165 word A-grade rewrite here","categories":{"headline":{"grade":"A|B|C|D|F","feedback":"one sentence"},"length":{"grade":"A|B|C|D|F","feedback":"one sentence"},"emotion":{"grade":"A|B|C|D|F","feedback":"one sentence"},"keywords":{"grade":"A|B|C|D|F","feedback":"one sentence"},"cta":{"grade":"A|B|C|D|F","feedback":"one sentence"},"compliance":{"grade":"A|B|C|D|F","feedback":"one sentence"}},"recommendations":["fix 1","fix 2","fix 3"]}`;
