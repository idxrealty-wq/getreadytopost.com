...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!resolvedUserDocId) {
      return NextResponse.json({ error: "User not found for provided userId/email" }, { status: 400 });
    }
    const userRef = db.collection("users").doc(resolvedUserDocId);
    const balanceRef = userRef.collection("credits").doc("balance");
    const submissionRef = submissionId ? db.collection("submissions").doc(String(submissionId)) : null;
    ...
    await logError({ source: "credits-deduct", error, context: { } });
