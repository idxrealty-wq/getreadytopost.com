                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              {listing.photos && listing.photos.length > 0 ? (
                                <img src={listing.photos.find((p: any) => p.downloadURL || p.url)?.downloadURL || listing.photos.find((p: any) => p.downloadURL || p.url)?.url || ""} alt="Property" className="w-14 h-14 object-cover rounded-lg border border-white/20 flex-shrink-0" />
                              ) : (
